...
export function parseFullAddress(input: string): ParsedAddress {
  ...
  if (!normalized) {
    throw new Error("A full address is required.");
  }
  ...
  if (!tailMatch) {
    throw new Error(
      "Address must end like: 123 Main St, Salt Lake City, UT 84101.",
    );
  }
  ...
  if (parts.length < 2) {
    throw new Error(
      "Address must include both a street address and city, separated by a comma.",
    );
  }
  ...
  if (!shortAddress) {
    throw new Error("Could not extract the short address.");
  }
